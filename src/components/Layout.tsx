...
import { Navigate, NavLink, useLocation, useNavigate, useOutlet } from 'react-router';
...
function useIsOwnerSession() {
  return useStore((s) => s.people.find((p) => p.id === s.sessionUserId)?.role === 'Owner');
}

function SessionMenu({ fallbackAvatar }: { fallbackAvatar: string }) {
  const navigate = useNavigate();
  const person = useStore((s) => s.people.find((p) => p.id === s.sessionUserId));
  ...
  return (
    <div className="relative">
      <button onClick={() => setOpen((v) => !v)} title={`${person.name} · ${role}`}>
        <img
          src={person.avatar ?? fallbackAvatar}
          alt={person.name}
          className="h-7 w-7 rounded-full border border-line-strong object-cover transition-colors hover:border-ember/60"
        />
      </button>
      {open && (
        <motion.div
          ...
          className="absolute right-0 top-10 z-50 w-56 rounded-panel border border-line-hairline bg-surface-1 p-3 shadow-modal"
        >
          <div className="text-small font-medium text-text-1">{person.name}</div>
          <div className="text-caption text-text-3">{person.title ?? role}</div>
          <button
            onClick={() => { signOut(); setOpen(false); navigate('/login'); }}
            className="mt-3 flex w-full items-center gap-2 rounded-chip border border-line-hairline px-2.5 py-1.5 text-caption text-text-2 transition-colors hover:border-danger/40 hover:text-danger"
          >
...
function TopBar() {
  const isOwnerSession = useIsOwnerSession();
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-40 flex h-14 items-center gap-4 border-b border-line-hairline bg-surface-1 px-4">
      ...
      <ThemeToggle />
      {isOwnerSession && <RoleSwitcher />}
      <SessionMenu fallbackAvatar={owner?.avatar ?? '/avatar-wanjiru.png'} />
    </header>
  );
}
        ...
        {visible.map((m, i) => {
          ...
          return (
            ...
            >
              ...
              {!m.resolved && action && action.status === 'pending' && canApprove && (
                <button
              ...
              {!m.resolved && action && action.status === 'pending' && !canApprove && (
                <div className="mt-2 text-caption text-text-3">Awaiting an Owner / Finance decision</div>
              )}
...
export default function Layout() {
  useTheme(); // re-render the whole shell (and all inline tint styles) on theme flip
  const location = useLocation();
